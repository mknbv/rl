...
class EnvBatch(Env):
  def __init__(self, make_env, num_envs=None):
    make_env_functions = self._get_make_env_functions(make_env, num_envs)
  ...
  def _get_make_env_functions(self, make_env, num_envs):
    if num_envs is None and not isinstance(make_env, list):
      raise ValueError("When num_envs is None make_env"
                       " must be a list of callables")
    if num_envs is not None and not callable(make_env):
      raise ValueError("When num_envs is not None make_env must be callable")

    if num_envs is not None:
      make_env = [make_env for _ in range(num_envs)]
    return make_env
...
class ParallelEnvBatch(EnvBatch):
  ...
  def __init__(self, make_env, num_envs=None):
    make_env_functions = self._get_make_env_functions(make_env, num_envs)
    self._num_envs = len(make_env_functions)
    self._parent_connections, self._worker_connections = zip(*[
      Pipe() for _ in range(self._num_envs)
    ])
    self._processes = [
        Process(
          target=worker,
